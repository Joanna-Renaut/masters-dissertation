...
approved_drugs = pandas.read_csv('../making-drug-databases/approved-drugs/12_approved_treatments_for_oesophageal_cancer.csv',
                                 header=0, sep=',')
...
approved_targets = drugbank.loc[drugbank[drugbank_drug].isin(approved_drugs[approved_drug])][[gene_name, drugbank_drug]]
...
oc_clinical_trials = pandas.read_csv('../00-database-csv/clinical_trials_.gov_OC_trials.csv', header=0, sep=',')
...
clinical_trial_drugs = pandas.read_csv('making-drug-databases-csv/temporary_ct_drugs.csv',
                                       header=0, sep=',')
drop_non_drugs = clinical_trial_drugs[clinical_trial_drugs['Interventions'].astype(str).str.startswith('Drug: ')]
...
remove_drug_prefix.to_csv('making-drug-databases-csv/clinical_trial_drugs_only.csv', index=False)


# # # 3. compare clinical trial drugs to drugbank to see if any different drug targets

clinical_trial_drugs = pandas.read_csv('../making-drug-databases/clinical-trial-drugs/12_clinical_trial_drugs_drugs_'
                                       'only.csv', header=0, sep=',')

ct_drug = 'Interventions'

ct_targets = drugbank.loc[drugbank[drugbank_drug].isin(clinical_trial_drugs[ct_drug])][[gene_name, drugbank_drug]]
# print(ct_targets)
...
cansar_SSL = pandas.read_csv('../making-drug-databases/cansar-drugs/12_adenocarcinoma_SSL_cpat_results.csv', header=0,
                             sep=',')
...
cansar_onco = pandas.read_csv('../making-drug-databases/cansar-drugs/12_squamous_cell_oncogenes_cpat_results.csv',
                              header=0, sep=',')
...
# # strip end
cansar_onco = cansar_onco['nearest_drug_target'].str.rstrip('|Whole:::Protein').to_frame()
...
print(cansar_onco)
cansar_onco.to_csv('../00-database-csv/squamous_cell_oncogenes_cansar_full.csv', index=False)


# # # 6. get cansar repositionable drugs for SSL targets squamous cell

cansar_SSL = pandas.read_csv('../making-drug-databases/cansar-drugs/12_squamous_cell_SSL_cpat_results.csv',
                             header=0, sep=',')
