...
five_percent_patients = adenocarcinoma.loc[
    adenocarcinoma[gene_name].isin(five_percent["Gene Name"])
][[patient, gene_name]]
# print(five_percent_patients)

full_patients = adenocarcinoma.loc[
    adenocarcinoma[patient].isin(five_percent_patients[patient])
][[patient]]
# print(full_patients)
...
fig.show()
fig.write_image(
    "violin-plots/adenocarcinoma_high_frequency_average_mutations_violin.png",
    width=1900,
    height=1080,
    scale=1,
)
...
squamous = pandas.read_csv(
    "../squamous-cell-carcinoma/squamous-cell-carcinoma-csv/04_filtered_cosmic_squamous_"
    "cell.csv",
    header=0,
    sep=",",
)
...
five_percent_patients = squamous.loc[
    squamous[gene_name].isin(five_percent["Gene Name"])
][[patient, gene_name]]
# print(five_percent_patients)

full_patients = squamous.loc[squamous[patient].isin(five_percent_patients[patient])][
    [patient]
]
# print(full_patients)
...
fig.show()
fig.write_image(
    "violin-plots/squamous_high_frequency_average_mutations_violin.png",
    width=1900,
    height=1080,
    scale=1,
)
...
total_distribution = pandas.concat(
    [oscc_count["OSCC Counts"], oac_count["OAC Counts"]], axis=1
)
# print(total_distribution)
total_distribution.to_csv(
    "../00-database-csv/total_mutation_distribution_data.csv", index=False
)


# # # 4. check average number of mutations in samples that contain a MUC16 mutation
muc_check_full_patients = adenocarcinoma.loc[
    adenocarcinoma[patient].isin(five_percent_patients[patient])
][[patient, gene_name]]
# print(muc_check_full_patients)
muc_count = muc_check_full_patients.groupby(" ID_SAMPLE")["GENE_NAME"].apply(
    lambda x: x[x == "MUC16"]
)
# print(muc_count)
mutational_load_filter = (
    muc_check_full_patients.groupby(" ID_SAMPLE")["GENE_NAME"]
...
compare = muc_check_full_patients.loc[
    muc_check_full_patients[" ID_SAMPLE"].isin(mutational_load_filter[" ID_SAMPLE"])
]
# print(compare)
...
no_muc_count = muc_check_full_patients.groupby(" ID_SAMPLE")["GENE_NAME"].apply(
    lambda x: x[x != "MUC16"]
)
# print(no_muc_count)
mutational_load_filter = (
