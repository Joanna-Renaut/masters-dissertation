...
high_frequency = pandas.read_csv('../../squamous-cell-carcinoma/squamous-cell-carcinoma-csv/04_squamous-cell-carcinoma_'
                                 'high_frequency_mutations(above_five_percent).csv', header=0, sep=',')
...
over_exp = expression_data.loc[expression_data[gene_name].isin(high_frequency[gene_name])][[gene_name, over_expressed]]
print(over_exp)
...
oncogenes = ts_onc_data.loc[ts_onc_data[gene_name].isin(over_exp[gene_name])][[gene_name, role_in_cancer]]
# print(oac_onc)
# print(type(oac_onc))
...
pathogenic = full_data.loc[full_data[gene_name].isin(filtered_onc[gene_name])][[gene_name, fathmm]]
# print(pathogenic)
...
onc_merge = pandas.merge(left=filtered_pathogenic, right=filtered_onc, how='outer', on='Gene Name')
# print(oac_onc_merge)
...
print(onc_merge)
onc_merge.to_csv('../over-under-expression-csv/24_squamous_cell_over_exp_oncogenes.csv',
                 index=False)
