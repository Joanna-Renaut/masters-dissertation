...
full_data = pandas.read_csv('../../adenocarcinoma/adenocarcinoma-csv/03_filtered_cosmic_adenocarcinoma.csv', header=0,
                            sep=',')
...
under_exp = expression_data.loc[expression_data[gene_name].isin(high_frequency[gene_name])][[gene_name, under_expressed]]
print(under_exp)
...
tumour_suppressors = ts_onc_data.loc[ts_onc_data[gene_name].isin(under_exp[gene_name])][[gene_name, role_in_cancer]]
# print(oac_onc)
# print(type(oac_onc))
tumour_suppressors.dropna(subset=['Role in Cancer'], how='any', inplace=True)
# print(oac_onc)
filtered_ts = tumour_suppressors[tumour_suppressors['Role in Cancer'].str.contains('TSG')]
# print(filtered_onc)


# # # 5. check if oncogene is pathogenic
pathogenic = full_data.loc[full_data[gene_name].isin(filtered_ts[gene_name])][[gene_name, fathmm]]
# print(pathogenic)
...
ts_merge = pandas.merge(left=filtered_pathogenic, right=filtered_ts, how='outer', on='Gene Name')
# print(oac_onc_merge)
...
print(ts_merge)
ts_merge.to_csv('../over-under-expression-csv/25_adenocarcinoma_under_exp_tumour_suppressors.csv',
                index=False)
