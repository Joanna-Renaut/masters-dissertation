...
oscc_data = pandas.read_csv('../over-under-expression-csv/26_squamous_cell_under_exp_tumour_suppressors.csv',
                            header=0, sep=',')
...
SSL_targets.to_csv('../over-under-expression-csv/28_squamous_cell_under_exp_synthetic_lethal_targets.csv', index=False)
...
squamous_cell_SSL_drugs = pandas.read_csv('../../00-database-csv/adenocarcinoma_SSL_cansar_full.csv', header=0, sep=',')

squamous_cell_SSL_targets = pandas.read_csv('../over-under-expression-csv/27_squamous_cell_under_exp_synthetic_lethal_'
                                            'targets.csv', header=0, sep=',')

compare = squamous_cell_SSL_targets.loc[squamous_cell_SSL_targets['Gene Name B'].isin(squamous_cell_SSL_drugs[
                                                                                            'Gene Name'])]
# print(compare)
compare_2 = squamous_cell_SSL_drugs.loc[squamous_cell_SSL_drugs['Gene Name'].isin(compare['Gene Name B'])]
# print(compare_2)
...
print(merge)
merge.to_csv('../../00-database-csv/squamous_cell_under_exp_SSL_druggable_targets.csv', index=False)