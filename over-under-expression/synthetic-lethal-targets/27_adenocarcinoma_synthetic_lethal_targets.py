...
oac_data = pandas.read_csv('../over-under-expression-csv/24_adenocarcinoma_under_exp_'
                           'tumour_suppressors.csv', header=0, sep=',')
...
SSL_targets.to_csv('../over-under-expression-csv/27_adenocarcinoma_under_exp_synthetic_lethal_targets.csv', index=False)
...
adenocarcinoma_SSL_drugs = pandas.read_csv('../../00-database-csv/adenocarcinoma_SSL_cansar_full.csv',
                                           header=0, sep=',')

adenocarcinoma_SSL_targets = pandas.read_csv('../over-under-expression-csv/26_adenocarcinoma_under_exp_synthetic_lethal'
                                             '_targets.csv', header=0, sep=',')

compare = adenocarcinoma_SSL_targets.loc[adenocarcinoma_SSL_targets['Gene Name B'].isin(adenocarcinoma_SSL_drugs[
                                                                                            'Gene Name'])]
# print(compare)
...
print(merge)
merge.to_csv('../../00-database-csv/adenocarcinoma_under_exp_SSL_druggable_targets.csv', index=False)