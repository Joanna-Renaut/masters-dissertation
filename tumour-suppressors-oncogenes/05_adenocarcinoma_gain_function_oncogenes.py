...
oac_gain = expression_data.loc[
    expression_data[gene_name].isin(oac_high_frequency[gene_name])
][[gene_name, gain_of_function]]
# print(oac_gain)
...
oac_onc = ts_onc_data.loc[ts_onc_data[gene_name].isin(oac_gain[gene_name])][
...
pathogenic = oac_full_data.loc[oac_full_data[gene_name].isin(filtered_onc[gene_name])][
    [gene_name, fathmm]
]
# print(pathogenic)
...
print(oac_onc_merge)
oac_onc_merge.to_csv(
    "../tumour-suppressors-oncogenes/tumour-suppressors-oncogenes-csv/"
    "05_adenocarcinoma_GOF_pathogenic_oncogenes.csv",
    index=False,
)
