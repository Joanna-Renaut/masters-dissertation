...
oscc_gain = expression_data.loc[
    expression_data[gene_name].isin(oscc_high_frequency[gene_name])
...
oscc_onc = ts_onc_data.loc[ts_onc_data[gene_name].isin(oscc_gain[gene_name])][
...
pathogenic = oscc_full_data.loc[
    oscc_full_data[gene_name].isin(filtered_onc[gene_name])
][[gene_name, fathmm]]
# print(pathogenic)
...
print(oscc_onc_merge)
oscc_onc_merge.to_csv(
    "../tumour-suppressors-oncogenes/tumour-suppressors-oncogenes-csv/"
    "06_squamous_cell_carcinoma_GOF_pathogenic_oncogenes.csv",
    index=False,
)
