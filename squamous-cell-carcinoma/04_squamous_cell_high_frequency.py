...
oscc_data = pandas.read_csv('squamous-cell-carcinoma-csv/02_cosmic_squamous_cell_only.csv',
...
filtered_data = oscc_data[~oscc_data[' MUTATION_AA'].str.contains('(?:p.\?)$') &
                          ~oscc_data[' MUTATION_DESCRIPTION'].str.contains('(?:Substitution - coding silent)$')]
# print(filtered_data.value_counts().head())
filtered_data.to_csv('../squamous-cell-carcinoma/squamous-cell-carcinoma-csv/04_filtered_cosmic_squamous_cell.csv',
                     index=False)
...
fig.write_image('../figures/histograms/high_frequency_squamous_cell.png', width=1920, height=1080, scale=1)
...
filtered_squamous = pandas.read_csv('../squamous-cell-carcinoma/squamous-cell-carcinoma-csv/04_filtered_cosmic_s'
                                          'quamous_cell.csv', header=0, sep=',')

patient = ' ID_SAMPLE'
gene_name = 'GENE_NAME'

five_percent_patients = filtered_squamous.loc[filtered_squamous[gene_name].isin(five_percent['Gene Name'])]
# print(five_percent_patients)

check_full_patients = filtered_squamous.loc[filtered_squamous[patient].isin(five_percent_patients[
                                                                                patient])][[patient, gene_name]]
# print(check_full_patients)

# # # FBXW7
mutational_load_filter = check_full_patients.groupby(' ID_SAMPLE')['GENE_NAME'].apply(lambda x: x[
    x == 'FBXW7']).to_frame().reset_index()
