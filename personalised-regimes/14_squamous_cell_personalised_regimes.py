...
repositionable_SSL_drugs = pandas.read_csv(
    "../00-database-csv/squamous_SSL_druggable_targets.csv", header=0, sep=","
)
repositionable_onco_drugs = pandas.read_csv(
    "../00-database-csv/squamous_cell_oncogenes_cansar_full.csv", header=0, sep=","
)
...
approved_vs_patients = approved_drugs.loc[
    approved_drugs["Gene Name"].isin(patients["GENE_NAME"])
][[gene_name, drugs, approved]]
...
patients_vs_approved = patients.loc[
    patients["GENE_NAME"].isin(approved_drugs["Gene Name"])
][[patient, gene, mutation_aa, mutation]]
# print(patients_vs_approved)


# # #  merge the columns to get lists of patient id, genes and drugs
merge_columns = approved_vs_patients.merge(
    patients_vs_approved, left_on="Gene Name", right_on="GENE_NAME"
)
# print(merge_columns)
...
approved_final.to_csv(
    "../personalised-regimes/personalised-regimes-csv/14_squamous_cell_approved_final.csv"
)
...
clinial_trial_vs_patients = clinical_trial_drugs.loc[
    clinical_trial_drugs["Gene Name"].isin(patients["GENE_NAME"])
...
mutation_aa = " MUTATION_AA"
mutation = " MUTATION_DESCRIPTION"
patients_vs_clinical_trial = patients.loc[
    patients["GENE_NAME"].isin(clinical_trial_drugs["Gene Name"])
][[patient, gene, mutation_aa, mutation]]
# print(patients_vs_approved)


# # #  merge the columns to get lists of patient id, genes and drugs
merge_columns = clinial_trial_vs_patients.merge(
    patients_vs_clinical_trial, left_on="Gene Name", right_on="GENE_NAME"
)
# print(merge_columns)
...
# print(ct_final)
ct_final.to_csv(
    "../personalised-regimes/personalised-regimes-csv/14_squamous_cell_clinical_trial_final.csv"
)
...
repositionable_vs_patients = repositionable_SSL_drugs.loc[
    repositionable_SSL_drugs["Gene Name A"].isin(patients["GENE_NAME"])
...
patients_vs_repositionable = patients.loc[
    patients["GENE_NAME"].isin(repositionable_SSL_drugs["Gene Name A"])
][[patient, gene, mutation_aa, mutation]]
# print(patients_vs_approved)

# # #  merge the columns to get lists of patient id, genes and drugs
merge_columns = repositionable_vs_patients.merge(
    patients_vs_repositionable, left_on="Gene Name A", right_on="GENE_NAME"
)
# print(merge_columns)
...
print(repositionable_SSL_final)
repositionable_SSL_final.to_csv(
    "../personalised-regimes/personalised-regimes-csv/14_squamous_cell_SSL_repositionable_"
    "final.csv"
)
...
repositionable_onco_final.to_csv(
    "../personalised-regimes/personalised-regimes-csv/14_squamous_cell_onco_"
...
ct_final = pandas.read_csv(
    "../personalised-regimes/personalised-regimes-csv/14_squamous_cell_clinical_trial_final.csv",
    header=0,
    sep=",",
)
repositionable_SSL_final = pandas.read_csv(
    "../personalised-regimes/personalised-regimes-csv/14_squamous_cell_SSL"
    "_repositionable_final.csv",
    header=0,
    sep=",",
)
repositionable_onco_final = pandas.read_csv(
    "../personalised-regimes/personalised-regimes-csv/14_squamous_cell_onco"
    "_repositionable_final.csv",
    header=0,
    sep=",",
)
...
# print(complete)
complete.replace(
    "Repositionable cancer drug - synthetic lethality",
    "Repositionable - SSL target",
    inplace=True,
)
complete.replace(
    "Repositionable cancer drug - GoF oncogene",
