...
"""
Compare approved drugs, clinical trial drugs, squamous cell oncogenes and squamous cell SSL drug targets to the patient 
data to find which patients are treatable
"""

patients = pandas.read_csv('../squamous-cell-carcinoma/squamous-cell-carcinoma-csv/04_filtered_cosmic_squamous_cell.csv'
                           , header=0, sep=',')
...
approved_final.to_csv('../personalised-regimes/personalised-regimes-csv/14_squamous_cell_approved_final.csv')
...
# print(ct_final)
ct_final.to_csv('../personalised-regimes/personalised-regimes-csv/14_squamous_cell_clinical_trial_final.csv')


# # # 3. compare repositionable SSL drugs against adenocarcinoma patients
repositionable_SSL_drugs.rename(columns={'Drugs': 'Drug Name', 'Gene Name B': 'SSL Partner'}, inplace=True)

gene_name = 'Gene Name A'
SSL_target = 'SSL Partner'
drugs = 'Drug Name'
respositionable = 'Drug Status'
repositionable_vs_patients = repositionable_SSL_drugs.loc[repositionable_SSL_drugs[
    'Gene Name A'].isin(patients['GENE_NAME'])][[gene_name, drugs, respositionable, SSL_target]]
# print(approved_vs_patients)
...
mutation_aa = ' MUTATION_AA'
mutation = ' MUTATION_DESCRIPTION'
patients_vs_repositionable = patients.loc[patients['GENE_NAME'].isin(repositionable_SSL_drugs[
                                                                         'Gene Name A'])][[patient, gene, mutation_aa,
                                                                                           mutation]]
# print(patients_vs_approved)
...
# # # # sort ID_SAMPLE by ID
repositionable_SSL_final = drop_column.set_index('Patient ID')
repositionable_SSL_final.reset_index()
repositionable_SSL_final.sort_values(by=['Patient ID'], inplace=True)
repositionable_SSL_final = repositionable_SSL_final.drop_duplicates()
print(repositionable_SSL_final)
repositionable_SSL_final.rename(columns={'Gene Name A': 'Gene Name'}, inplace=True)
print(repositionable_SSL_final)
repositionable_SSL_final.to_csv('../personalised-regimes/personalised-regimes-csv/14_squamous_cell_SSL_repositionable_'
                                'final.csv')
...
drugs = 'Drug Name'
respositionable = 'Drug Status'
repositionable_vs_patients = repositionable_onco_drugs.loc[repositionable_onco_drugs[
    'Gene Name'].isin(patients['GENE_NAME'])][[gene_name, drugs, respositionable]]
# print(approved_vs_patients)
...
mutation_aa = ' MUTATION_AA'
mutation = ' MUTATION_DESCRIPTION'
patients_vs_repositionable = patients.loc[patients['GENE_NAME'].isin(repositionable_onco_drugs[
                                                                         'Gene Name'])][[patient, gene, mutation_aa,
                                                                                         mutation]]
# print(patients_vs_approved)

# # #  merge the columns to get lists of patient id, genes and drugs
merge_columns = repositionable_vs_patients.merge(patients_vs_repositionable, left_on='Gene Name', right_on='GENE_NAME')
# print(merge_columns)
...
repositionable_onco_final.to_csv('../personalised-regimes/personalised-regimes-csv/14_squamous_cell_onco_'
                                 'repositionable_final.csv')


# # # 5. concatenate the data into one file

approved_final = pandas.read_csv('../personalised-regimes/personalised-regimes-csv/14_squamous_cell_approved_final.csv'
                                 , header=0, sep=',')
ct_final = pandas.read_csv('../personalised-regimes/personalised-regimes-csv/14_squamous_cell_clinical_trial_final.csv'
                           , header=0, sep=',')
repositionable_SSL_final = pandas.read_csv('../personalised-regimes/personalised-regimes-csv/14_squamous_cell_SSL'
                                       '_repositionable_final.csv', header=0, sep=',')
repositionable_onco_final = pandas.read_csv('../personalised-regimes/personalised-regimes-csv/14_squamous_cell_onco'
                                       '_repositionable_final.csv', header=0, sep=',')
...
print(complete)
complete.to_csv('FINAL_squamous_cell_complete_patient_regimes.csv', index=False)
